# Amazon Bedrock Prompt Engineering Demo - Components Only
...
def parameter_sidebar():
    ...
    with st.sidebar:
        ...
        # Calculate time since first message if there's at least one timestamp
        if st.session_state.response_timestamps:
            session_time = int(time.time() - st.session_state.response_timestamps[0])
            st.markdown(f"**Session Time:** {session_time}s")
        
        col1, col2 = st.columns(2)
        with col1:
            st.button("Reset Session", key="reset_session", on_click=reset_session, type="primary")
        
        st.markdown("</div>", unsafe_allow_html=True)
        
        with st.expander("About this Demo", expanded=False):
            st.markdown("""
            This app demonstrates the essential components of effective prompt engineering
...
def instruction_tab(model_id, params):
    ...
    # Information box
    with st.expander("Learn about Instruction Components", expanded=True):
        st.markdown("""
...
def context_tab(model_id, params):
    ...
    # Information box
    with st.expander("Learn about Context Components", expanded=True):
        st.markdown("""
...
def input_data_tab(model_id, params):
    ...
    # Information box
    with st.expander("Learn about Input Data Components", expanded=True):
        st.markdown("""
...
def output_indicator_tab(model_id, params):
    ...
    # Information box
    with st.expander("Learn about Output Indicator Components", expanded=True):
        st.markdown("""
