...
def parameter_sidebar():
    ...
    with st.sidebar:
        st.markdown("<div class='sidebar-header'>Prompt Engineering Exercises</div>", unsafe_allow_html=True)
        ...
        reset_button = st.button("Reset Session", key="reset_session", type="primary", 
                                help="Clear all responses and conversation history")
        if reset_button:
            reset_session()
            st.rerun()
...
def main():
    ...
    for tab_idx, (tutorial_id, tutorial_info) in enumerate(tutorials.items()):
        with tutorial_tabs[tab_idx]:
            ...
            for exercise in tutorial_info['exercises']:
                ...
                # Buttons for submitting and showing expected result
                ...
                with col2:
                    show_expected = st.button("Show Expected Result", key=f"expected_{exercise_id}")
    ...
    <div style="text-align: center">
        <p>Based on the Prompt Engineering Playbook (Beta v3)</p>
        <p>Practice makes perfect! Keep refining your prompts to get better results.</p>
    </div>
    """, unsafe_allow_html=True)
