# Amazon Bedrock Guardrail Stream - Enhanced Streamlit Application

"""
Streamlit application demonstrating Amazon Bedrock Guardrails
with the ConverseStream operation.
"""
...
def parameter_sidebar():
    ...
    with st.sidebar:
        # st.markdown("<div class='sidebar-header'><h2>🛡️ Bedrock Guardrail</h2></div>", unsafe_allow_html=True)
        ...
        with st.expander("Guardrail Settings", expanded=True):
            guardrail_id = st.text_input("Guardrail ID", value="efzsheyu2yvm", key="guardrail_id")
            guardrail_version = st.text_input("Guardrail Version", value="3", key="guardrail_version")
...
def main():
    ...
    if submit_button and user_input:
        ...
        if bedrock_client:
            ...
            # Save to history
            st.session_state.history[st.session_state.conversation_id] = st.session_state.conversation
    ...
    # Footer
    st.markdown("---")
    st.caption("Amazon Bedrock Guardrail Demo • Built with Streamlit")
