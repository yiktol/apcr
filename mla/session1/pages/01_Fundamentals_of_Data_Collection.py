...
st.set_page_config(page_title="AWS Data Structures & Storage", 
                   layout="wide",
...
with tabs[0]:
    ...
    # Draw arrows
    ax.arrow(nodes['structured'][0], nodes['structured'][1], 
             nodes['rds'][0]-nodes['structured'][0]-0.02, 0, 
             head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
    
    ax.arrow(nodes['semi'][0], nodes['semi'][1], 
             nodes['dynamo'][0]-nodes['semi'][0]-0.02, 0, 
             head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
    
    ax.arrow(nodes['unstructured'][0], nodes['unstructured'][1], 
             nodes['s3'][0]-nodes['unstructured'][0]-0.02, 0, 
             head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
    
    ax.arrow(nodes['rds'][0], nodes['rds'][1], 
             nodes['redshift'][0]-nodes['rds'][0]-0.02, nodes['redshift'][1]-nodes['rds'][1], 
             head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
    
    ax.arrow(nodes['dynamo'][0], nodes['dynamo'][1], 
             nodes['ml'][0]-nodes['dynamo'][0]-0.02, nodes['ml'][1]-nodes['dynamo'][1], 
             head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
    
    ax.arrow(nodes['s3'][0], nodes['s3'][1], 
             nodes['ml'][0]-nodes['s3'][0]-0.02, nodes['ml'][1]-nodes['s3'][1], 
             head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
...
with tabs[2]:
    ...
    with comparison_tabs[0]:
        ...
        # Draw arrows
        ax.arrow(pos['sources'][0], pos['sources'][1], 
                pos['database'][0]-pos['sources'][0]-0.02, pos['database'][1]-pos['sources'][1], 
                head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
        
        ax.arrow(pos['sources'][0], pos['sources'][1], 
                pos['warehouse'][0]-pos['sources'][0]-0.02, pos['warehouse'][1]-pos['sources'][1], 
                head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
        
        ax.arrow(pos['sources'][0], pos['sources'][1], 
                pos['lake'][0]-pos['sources'][0]-0.02, pos['lake'][1]-pos['sources'][1], 
                head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
        
        ax.arrow(pos['database'][0], pos['database'][1], 
                pos['app'][0]-pos['database'][0]-0.02, 0, 
                head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
        
        ax.arrow(pos['warehouse'][0], pos['warehouse'][1], 
                pos['bi'][0]-pos['warehouse'][0]-0.02, 0, 
                head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
        
        ax.arrow(pos['lake'][0], pos['lake'][1], 
                pos['ml'][0]-pos['lake'][0]-0.02, 0, 
                head_width=0.02, head_length=0.02, fc=AWS_COLORS['dark_blue'], ec=AWS_COLORS['dark_blue'])
    ...
    with comparison_tabs[1]:
        ...
        # Data sources
        draw_box(0.05, 0.7, 0.15, 0.2, AWS_COLORS['orange'], 'Structured\nData')
        draw_box(0.05, 0.4, 0.15, 0.2, AWS_COLORS['teal'], 'Semi-structured\nData')
        draw_box(0.05, 0.1, 0.15, 0.2, AWS_COLORS['purple'], 'Unstructured\nData')
        
        # Ingestion
        draw_box(0.3, 0.4, 0.15, 0.2, AWS_COLORS['light_blue'], 'Data\nIngestion\n(Kinesis, Transfer)')
        
        # Storage
        draw_box(0.55, 0.4, 0.15, 0.2, AWS_COLORS['dark_blue'], 'Amazon S3\nData Lake')
        
        # Processing
        draw_box(0.8, 0.7, 0.15, 0.2, AWS_COLORS['light_blue'], 'AWS Glue\n(Catalog & ETL)')
        draw_box(0.8, 0.4, 0.15, 0.2, AWS_COLORS['light_blue'], 'EMR\n(Processing)')
        draw_box(0.8, 0.1, 0.15, 0.2, AWS_COLORS['light_blue'], 'Athena\n(Query)')
        ...
        draw_arrow(0.2, 0.8, 0.3, 0.5)
        draw_arrow(0.2, 0.5, 0.3, 0.5)
        draw_arrow(0.2, 0.2, 0.3, 0.5)
        
        # Ingestion to storage
        draw_arrow(0.45, 0.5, 0.55, 0.5)
        
        # Storage to processing
        draw_arrow(0.7, 0.5, 0.8, 0.8)
        draw_arrow(0.7, 0.5, 0.8, 0.5)
        draw_arrow(0.7, 0.5, 0.8, 0.2)
    ...
    with comparison_tabs[2]:
        ...
        # Data sources
        draw_box(0.05, 0.6, 0.15, 0.15, AWS_COLORS['orange'], 'ERP System')
        draw_box(0.05, 0.4, 0.15, 0.15, AWS_COLORS['orange'], 'CRM System')
        draw_box(0.05, 0.2, 0.15, 0.15, AWS_COLORS['orange'], 'Other Systems')
        
        # ETL
        draw_box(0.3, 0.4, 0.15, 0.2, AWS_COLORS['light_blue'], 'AWS Glue\n(ETL)')
        
        # Data Warehouse
        draw_box(0.55, 0.3, 0.15, 0.4, AWS_COLORS['teal'], 'Amazon\nRedshift\nData\nWarehouse')
        
        # BI and Reporting
        draw_box(0.8, 0.6, 0.15, 0.15, AWS_COLORS['green'], 'QuickSight\n(BI)')
        draw_box(0.8, 0.4, 0.15, 0.15, AWS_COLORS['green'], 'Business\nReporting')
        draw_box(0.8, 0.2, 0.15, 0.15, AWS_COLORS['green'], 'Executive\nDashboards')
        
        # Connect boxes with arrows
        # Sources to ETL
        draw_arrow(0.2, 0.675, 0.3, 0.5)
        draw_arrow(0.2, 0.475, 0.3, 0.5)
        draw_arrow(0.2, 0.275, 0.3, 0.5)
        
        # ETL to warehouse
        draw_arrow(0.45, 0.5, 0.55, 0.5)
        
        # Warehouse to BI tools
        draw_arrow(0.7, 0.6, 0.8, 0.675)
        draw_arrow(0.7, 0.5, 0.8, 0.475)
        draw_arrow(0.7, 0.4, 0.8, 0.275)
    ...
    with comparison_tabs[3]:
        ...
        # Applications
        draw_box(0.05, 0.6, 0.15, 0.15, AWS_COLORS['green'], 'Web App')
        draw_box(0.05, 0.4, 0.15, 0.15, AWS_COLORS['green'], 'Mobile App')
        draw_box(0.05, 0.2, 0.15, 0.15, AWS_COLORS['green'], 'Internal\nSystems')
        
        # Load Balancer
        draw_box(0.3, 0.4, 0.15, 0.2, AWS_COLORS['light_blue'], 'Application\nLoad Balancer')
        
        # Database instances
        draw_box(0.55, 0.5, 0.15, 0.2, AWS_COLORS['orange'], 'Primary\nDB Instance')
        draw_box(0.55, 0.2, 0.15, 0.2, AWS_COLORS['light_orange'], 'Replica\nDB Instance')
        
        # Services using DB
        draw_box(0.8, 0.6, 0.15, 0.15, AWS_COLORS['purple'], 'User\nAuthentication')
        draw_box(0.8, 0.4, 0.15, 0.15, AWS_COLORS['purple'], 'Order\nProcessing')
        draw_box(0.8, 0.2, 0.15, 0.15, AWS_COLORS['purple'], 'Inventory\nManagement')
        ...
        draw_arrow(0.45, 0.5, 0.55, 0.6)
        
        # Primary to replica
        draw_arrow(0.625, 0.5, 0.625, 0.4)
        
        # Databases to services
        draw_arrow(0.7, 0.6, 0.8, 0.675)
        draw_arrow(0.7, 0.6, 0.8, 0.475)
        draw_arrow(0.7, 0.3, 0.8, 0.275)
...
#    - Uses AWS color scheme with orange, dark blue, teal, etc.
#    - Card-based layout with clean design
#    - Tab-based navigation for easy content access

# 2. **Tab 1: Data Structures**:
#    - Explains structured, semi-structured, and unstructured data
#    - Includes examples and visualizations of each data type
#    - Data processing flow visualization

# 3. **Tab 2: Storage Formats**:
#    - Comparison of common storage formats (CSV, Parquet, JSON, etc.)
#    - Interactive performance visualization
#    - Detailed code examples of each format
#    - AWS service recommendations

# 4. **Tab 3: Data Warehouses vs Data Lakes**:
#    - Detailed comparison of databases, data warehouses, and data lakes
...
#    - Clean initialization of session state variables

# The application is optimized for web access with responsive design and carefully crafted visualizations using Matplotlib and Altair.